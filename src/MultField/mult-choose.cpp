...
int num = 0, A_num = 0, B_num = 0, C_num = 0;
...
inline void import(const char *localFileName) {
    ...
    num = 0;
    num += input["portals"]["idA"]["bkmrk"].getMemberNames().size();
	...
	for(Json::Value::Members::iterator iter = arrayMember.begin(); iter != arrayMember.end(); ++iter) {
		PortalA[A_num].guid = input["portals"]["idA"]["bkmrk"][*iter]["guid"].asString();
		PortalA[A_num].latlng = input["portals"]["idA"]["bkmrk"][*iter]["latlng"].asString();
		PortalA[A_num].label = input["portals"]["idA"]["bkmrk"][*iter]["label"].asString();
		PortalA[A_num].to_xyz();
        A_num += 1;
	}
    if (A_num == 0) {
	...
	for(Json::Value::Members::iterator iter = arrayMember.begin(); iter != arrayMember.end(); ++iter) {
		PortalB[B_num].guid = input["portals"]["idB"]["bkmrk"][*iter]["guid"].asString();
		PortalB[B_num].latlng = input["portals"]["idB"]["bkmrk"][*iter]["latlng"].asString();
		PortalB[B_num].label = input["portals"]["idB"]["bkmrk"][*iter]["label"].asString();
		PortalB[B_num].to_xyz();
        B_num += 1;
	}
    if (B_num == 0) {
	...
	for(Json::Value::Members::iterator iter = arrayMember.begin(); iter != arrayMember.end(); ++iter) {
		PortalC[C_num].guid = input["portals"]["idOthers"]["bkmrk"][*iter]["guid"].asString();
		PortalC[C_num].latlng = input["portals"]["idOthers"]["bkmrk"][*iter]["latlng"].asString();
		PortalC[C_num].label = input["portals"]["idOthers"]["bkmrk"][*iter]["label"].asString();
		PortalC[C_num].to_xyz();
        C_num += 1;
	}
}
...
int main() {
    ...
    for (int i = 0; i < (int)q.size(); i++) {
        ...
        maxLayer = max(maxLayer, layer[q[i].second]);
    }
    for (int i = 0; i < C_num; i++) if (layer[i] == maxLayer)
    {
        addPortal(PortalC[i]);
        addLink(PortalA[0], PortalC[i]);
        addLink(PortalB[0], PortalC[i]);
        while (layer[i] != 1) {
            addLink(PortalC[i], PortalC[nxt[i]]);
            i = nxt[i];
            addPortal(PortalC[i]);
            addLink(PortalA[0], PortalC[i]);
            addLink(PortalB[0], PortalC[i]);
